...
from states import StorageFunctions, Alert
...
@dp.message_handler(IsAdmin(), text=["📢 Рассылка","📢 Mailing"], state="*")
async def send_ad_all_users(message: types.Message, state: FSMContext):
    await state.finish()
    if lang(message.from_user.id)=='Eng':
        await message.answer("📢 <b>Enter the text to send to users:</b>")
    else:
        await message.answer("📢 <b>Введите текст для рассылки пользователям:</b>")
    await Alert.here_ad_text.set()


# Обработка кнопки "Поиск профиля"
@dp.message_handler(IsAdmin(), text=["📱 Поиск профиля 🔍","📱 Profile Search 🔍"], state="*")
async def search_profile(message: types.Message, state: FSMContext):
    await state.finish()
    if lang(message.from_user.id)=='Eng':
        await message.answer("<b>📱 Enter the username or user ID. Example:</b>\n"
                            "▶ 123456789\n"
                            "▶ @example")
    else:
...
@dp.message_handler(IsAdmin(), state=Alert.here_ad_text)
async def input_text_for_ad(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if lang(message.from_user.id)=='Eng':
            data["here_send_ad"] = "📢 Mailing.\n" + str(message.text)
        else:
    ...
    if lang(message.from_user.id)=='Eng':
        await bot.send_message(message.from_user.id,
                            f"📢 Do you want to send a message:\n"
                            f"▶ <code>{message.text}</code>\n"
                            f"👤 <code>{len(users)}</code> пользователям?",
                            reply_markup=sure_send_ad_inl(message.from_user.id))
    else:
        await bot.send_message(message.from_user.id,
...
@dp.message_handler(IsAdmin(), state=Alert.Button)
async def input_text_for_ad(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        send_ad_message = data["here_send_ad"]
    send_button_message=message.text.split(',')
    await state.finish()
    asyncio.create_task(send_message_to_user(send_ad_message, message.from_user.id, send_button_message))

@dp.callback_query_handler(IsAdmin(), text=["not_send_kb", "yes_send_ad",'button_send_ad'], state=Alert.here_ad_text)
async def sends_ad(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    if lang(call.from_user.id)=='Eng':
        if call.data == "not_send_kb":
            await state.finish()
            await call.message.answer("<b>📢 You have canceled ☑</b>")
        elif call.data == "button_send_ad":
            await Alert.next()

            await call.message.answer("<b>Enter the buttons in the form:</b>\nText - https://example.com, Text2 - https://example2.com")
        else:
            await call.message.answer(f"<b>📢 Mailing started...</b>")
            async with state.proxy() as data:
                send_ad_message = data["here_send_ad"]
            await state.finish()
        ...
        if call.data == "not_send_kb":
            ...
            await Alert.next()
            await call.message.answer("<b>Введите кнопки в виде:</b>\nText - https://example.com, Text2 - https://example2.com")
        else:
...
# Отправка сообщений
async def send_message_to_user(message, user_id, send_button_message=0):
    if send_button_message:
        buttons = InlineKeyboardMarkup()
        k=0
        for i in (send_button_message):
            k+=1
            i=i.split('-')
            button = InlineKeyboardButton(text=i[0].strip(), url=i[1].strip())
            if k%2==0:
        ...
        for user in users:
            try:
                await bot.send_message(user[1], message, reply_markup=buttons)
                receive_users += 1
            except:
                block_users += 1
        ...
        if lang(user_id)=='Eng':
            await bot.send_message(user_id,
                                f"<b>📢 The mailing has been completed ☑</b>\n"
                                f"👤 Users received a message: <code>{receive_users} ✅</code>\n"
                                f"👤 Users did not receive the message: <code>{block_users} ❌</code>")
        else:
            await bot.send_message(user_id,
...
async def input_data_for_search_profile(message: types.Message, state: FSMContext):
    ...
    if get_user_id is not None:
        ...
        if lang(message.from_user.id)=='Eng':
            await message.answer("<b>❌ The profile was not found</b>\n"
                                "📱 Enter the username or user ID. Example:\n"
                                "▶ 123456789\n"
                                "▶ @example")
        else:
...
async def change_user_sale(call: CallbackQuery, state: FSMContext):
    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            if lang(call.from_user.id)=='Eng':
                save_purchases.append(f"<b>📃 Receipt:</b> <code>#{purchases[4]}</code>\n"
                                    f"▶ {purchases[9]} | {purchases[5]} | {purchases[6]}$\n"
                                    f"🕜 {purchases[13]}\n"
        ...
        if lang(call.from_user.id)=='Eng':
            await bot.answer_callback_query(call.id, "❗ The user has no purchases")
        else:
...
async def add_balance_user(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    if lang(call.from_user.id)=='Eng':
        await call.message.answer("<b>💴 Enter the amount to issue the balance</b>")
    else:
...
async def input_add_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 1:
            ...
            update_userx(user_id, balance=float(get_user[4]) + get_amount)
            if lang(message.from_user.id)=='Eng':
                await message.answer("<b>✅ To the user</b> "
                                    f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
                                    f"<b>was issued</b> <code>{get_amount}$</code>",
                ...
                await message.answer("<b>✅ Пользователю</b> "
                                    ...
                                    reply_markup=check_user_out_func(message.from_user.id))
            await bot.send_message(user_id, f"<b>💳 You have been issued</b> <code>{get_amount}$</code>")
            await message.answer(search_user_profile(user_id), reply_markup=search_profile_func(user_id))
            await state.finish()
        else:
            if lang(message.from_user.id)=='Eng':
                await message.answer("<b>❌ Minimum issue amount 1 $</b>\n"
                                    "💴 Enter the amount to issue the balance")
            else:
...
async def set_balance_user(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    if lang(call.from_user.id)=='Eng':
        await call.message.answer("<b>💸 Enter the amount to change the balance</b>")
    else:
    ...
    if message.text.isdigit():
        ...
        if get_amount >= 0:
            ...
            if lang(message.from_user.id)=='Eng':
                await message.answer("<b>✅ To the user</b> "
                                    f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
                                    f"<b>the balance was changed to</b> <code>{get_amount}$</code>",
                                    reply_markup=check_user_out_func(message.from_user.id))
            else:
                await message.answer("<b>✅ Пользователю</b> "
                        f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
            ...
            if lang(message.from_user.id)=='Eng':
                await message.answer("<b>❌ Minimum balance amount 0 $</b>\n"
                                    "💸 Enter the amount to change the balance")
            else:
...
async def send_user_message(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    if lang(call.from_user.id)=='Eng':
        await call.message.answer("<b>💌 Enter the message to send</b>\n"
                                "⚠ The message will be immediately sent to the user.")
    else:
...
async def input_send_user_message(message: types.Message, state: FSMContext):
    get_message = "<b>❕ You have a message:</b>\n" + message.text
    async with state.proxy() as data:
        user_id = data["here_cache_user_id"]
    get_user = get_userx(user_id=user_id)
    ...
    if message.text.startswith("+"):
        ...
        if get_purchase is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> <code>#{get_purchase[4]}</code>\n"
                                f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                f"🏷 Название товаров:\n<code>{get_purchase[6]}</code>\n"
                                f"💵 Сумма покупки: <code>{get_purchase[5]}$</code>\n"
                                f"👤 Купил товар: <a href='tg://user?id={get_purchase[1]}'>{get_purchase[3]}</a> <code>({get_purchase[1]})</code>\n"
                                f"🔻 Баланс до покупки: <code>{get_purchase[7]}$</code>\n"
                                f"🔺 Баланс после покупки: <code>{get_purchase[8]}$</code>\n"
                                f"🕜 Дата покупки: <code>{get_purchase[9]}</code>\n"
                                f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n",
                                reply_markup=get_functions_func(message.from_user.id))
        else:
            await message.answer("<b>❌ Чек не был найден.</b>\n"
